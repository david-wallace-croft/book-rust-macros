...
// For test_ch08_p170_naming
#[proc_macro_derive(
  BuilderNaming,
  attributes(rename)
)]
pub fn builder_naming(item: TokenStream) -> TokenStream {
  create_builder_naming(item.into()).into()
}

// For test_ch08_p173_sensible
#[proc_macro_derive(
  BuilderSensible,
  attributes(
    builder_defaults,
...
pub fn builder_build(item: TokenStream) -> TokenStream {
  create_builder_build(item.into()).into()
}

// For test_ch08_p201_ex1
#[proc_macro_derive(
  BuilderEx1,
  attributes(builder)
)]
...
// For test_ch09_p215_struct
#[proc_macro]
pub fn iac_struct(item: TokenStream) -> TokenStream {
  let ii: IacInputStruct = parse_macro_input!(item);
...
// For test_ch09_p218_using
#[proc_macro]
pub fn iac_using(item: TokenStream) -> TokenStream {
  let ii: IacInputUsing = parse_macro_input!(item);
...
// For test_ch10_p258_ex1
#[proc_macro_attribute]
pub fn config_ex1(
  attr: TokenStream,
  item: TokenStream,
) -> TokenStream {
  let config_input_ex1: ConfigInputEx1 = parse_macro_input!(attr);

  let ast: DeriveInput = parse_macro_input!(item);

  let exclude_from_method: bool = config_input_ex1.exclude_from;

  match find_yaml_values_ex1(config_input_ex1) {
    Ok(values) => {
