...
pub fn find_yaml_values_struct(
  config_input_struct: ConfigInputStruct
) -> Result<HashMap<String, String>, syn::Error> {
  let file_name: String = config_input_struct
    .path
...
#[derive(Debug)]
pub struct ConfigInputStruct {
  pub path: Option<String>,
}

impl Parse for ConfigInputStruct {
  fn parse(input: ParseStream) -> syn::Result<Self> {
    if input.is_empty() {
      return Ok(ConfigInputStruct {
        path: None,
      });
    }
